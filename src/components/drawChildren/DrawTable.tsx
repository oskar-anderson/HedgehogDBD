...
function DrawTableRow({ row, height, tableName }: DrawTableRowProps) {
...
function DrawTableRowWrapper( { row, rowStartY, tableName, tableId, handleStyle, children }: DrawTableRowWrapperProps) {
    return (
        <div 
            onContextMenu={ (e) => { 
                e.preventDefault();
                e.stopPropagation();
                publish("e_openTableContextMenu", { event: e, row, tableId }); 
            }} 
        >
...
const tableStyle = {
    headingStyle:  { 
        ...
        border: `solid ${tableConstants.innerBorderWidth}px #dee5ee`, 
        padding: `${tableConstants.headingPaddingY}px 12px`, 
        backgroundColor: "#f1f6f8"
    },
    contentBorderStyle: {
        borderLeft: `solid ${tableConstants.innerBorderWidth}px #dee5ee`,
        borderRight: `solid ${tableConstants.innerBorderWidth}px #dee5ee`,
        borderBottom: `solid ${tableConstants.innerBorderWidth}px #dee5ee`
    }
}
        ...
        <>
            <div style={{  
                ...
                <div style={{ borderRadius: "4px",  backgroundColor: "#eee" }}>
                    ...
                    <div className="d-flex" style={tableStyle.headingStyle}>
                        <div className="w-100 d-flex justify-content-center" style={{ fontWeight: "500" }}>{name}</div>
                    </div>

                    <div style={{ borderRadius: "0 0 4px 4px", backgroundColor: "white", padding: `0 4px`, ...tableStyle.contentBorderStyle }}>
                        {
                            tableRows.map((row, i) => {
                                const rowStartY = 
                                (tableConstants.outerBorderWidth + 2 * tableConstants.innerBorderWidth + tableConstants.rowHeight + 2*tableConstants.headingPaddingY) + 
                                (tableConstants.rowHeight / 2) + (i * tableConstants.rowHeight) + tableConstants.alignCenterTweak;  
                                return (
                                    <DrawTableRow key={row.name} tableName={name} row={row} height={tableConstants.rowHeight}></DrawTableRow>
                                );
                            })
                        }
                    </div>
...
export default function DrawTable(node: Pick<NodeProps<NodePayload>, "data" | "selected"> ) {
    const tableId = node.data.table.id;
    const tableName = node.data.table.head;
    const tableRows = node.data.table.tableRows;
    const selected = node.selected;

    const tableY = tableConstants.outerBorderWidth + 2 * tableConstants.innerBorderWidth + tableConstants.headingPaddingY + tableConstants.rowHeight/2 + tableConstants.alignCenterTweak;

    return (
        <>
            <div style={{  
                borderRadius: "6px", 
                border: `solid ${tableConstants.outerBorderWidth}px ${selected ? '#5a67d8' : 'transparent'}`,
                width: "min-content"
            }} onClick={(e) => {
                e.stopPropagation();
                publish("e_clickedOnTable", { event: e, targetTableId: tableId}) 
            }}>
                <div style={{ borderRadius: "4px",  backgroundColor: "#eee" }}>
                    ...
                    <div className="d-flex" style={tableStyle.headingStyle} 
                        onContextMenu={(e) => {
                            e.stopPropagation();
                            e.preventDefault();
                            publish("e_openTableContextMenu", { event: e, tableId: tableId, row: undefined }) 
                        }}
                    >
                        <Handle type="source" id={`${tableName}-head-left`} position={Position.Left} style={{ top: `${tableY}px`, left: `calc(3px - ${EdgeNotationPadding}px)`, ...handleStyle }} />
            
                        <div className="w-100 d-flex justify-content-center" style={{ fontWeight: "500" }}>{tableName}</div>
                    
                        <Handle type="source" id={`${tableName}-head-right`} position={Position.Right} style={{ top: `${tableY}px`, right: `calc(3px - ${EdgeNotationPadding}px)`, ...handleStyle}} />
                    </div>
                    ...
                    <div style={{ borderRadius: "0 0 4px 4px", backgroundColor: "white", padding: `0 4px`, ...tableStyle.contentBorderStyle }}>
                        {
                            tableRows.map((row, i) => {
                                const rowStartY = 
                                    ...
                                    (tableConstants.rowHeight / 2) + (i * tableConstants.rowHeight) + tableConstants.alignCenterTweak;  
                                return (
                                    <DrawTableRowWrapper key={row.name} handleStyle={handleStyle} tableName={tableName} tableId={tableId} row={row} rowStartY={rowStartY}>
                                        <DrawTableRow tableName={tableName} row={row} height={tableConstants.rowHeight}></DrawTableRow>
                                    </DrawTableRowWrapper>
                                );
                            })
                        }
