...
function DrawTableRow( { row, rowStartY, height, tableName, tableId }: DrawTableRowProps) {
    ...
    const foreignKeyRelation = relations
    ...
    if (
        (tableName === "vehicle" && row.name === "id") || 
        (tableName === "route" && row.name === "vehicle_id")
        ) {
        console.log("rerendered", tableName, "target", "left", tableName, "row", row.name, 
        relations.filter(relation => 
            relation.target.id === tableId
            && relation.targetRow.name === row.name
            && relation.source.position.x < relation.target.position.x
            )
        )
        console.log("rerendered", tableName, "target", "right", tableName, "row", row.name, 
        relations.filter(relation => 
            relation.target.id === tableId
            && relation.targetRow.name === row.name
            && relation.source.position.x >= relation.target.position.x
        ...
        console.log("rerendered", tableName, "source", "left", tableName, "row", row.name, 
        relations.filter(relation => 
            relation.source.id === tableId
            && relation.sourceRow.name === row.name
            && relation.source.position.x > relation.target.position.x
            )
        )
        console.log("rerendered", tableName, "source", "right", tableName, "row", row.name, 
        relations.filter(relation => 
            relation.source.id === tableId
            && relation.sourceRow.name === row.name
            && relation.source.position.x <= relation.target.position.x
        ...
        <div>
            { relations.filter(relation => 
                ...
                && relation.source.position.x > relation.target.position.x
                    ...
                    const key = `${relation.source.head}(${relation.sourceRow.name}) ref ${relation.target.head}(${relation.targetRow.name}) - left`;
                    return (
                        <Handle key={key} type="source" id={key} position={Position.Left} style={{ top: `${rowStartY}px`, left: '2px', ...handleStyle }} />            
                    )
                })
            }
            { relations.filter(relation => 
                ...
                && relation.source.position.x < relation.target.position.x
                    ...
                    const key = `${relation.source.head}(${relation.sourceRow.name}) ref ${relation.target.head}(${relation.targetRow.name}) - left`;
                    return (
                        <Handle key={key} type="target" id={key} position={Position.Left} style={{ top: `${rowStartY}px`, left: '2px', ...handleStyle }} />            
                    )
                })
            }
            ...
            <div className='d-flex' style={{ height: `${height}px`, whiteSpace: "nowrap" }}>
                <div className='d-flex align-items-center' style={{ width: "16px", paddingRight: "4px" }}>
                    ...
                    {
                        foreignKeyRelation ?
                        <span title={`FK(${foreignKeyRelation.target.head})`}>
            ...
            { relations.filter(relation => 
                ...
                && relation.source.position.x <= relation.target.position.x
                    ...
                    const key = `${relation.source.head}(${relation.sourceRow.name}) ref ${relation.target.head}(${relation.targetRow.name}) - right`;
                    return (
                        <Handle key={key} type="source" id={key} position={Position.Right} style={{ top: `${rowStartY}px`, right: '2px', ...handleStyle }} />            
                    )
                })
            }
            { relations.filter(relation => 
                ...
                && relation.source.position.x >= relation.target.position.x
                    ...
                    const key = `${relation.source.head}(${relation.sourceRow.name}) ref ${relation.target.head}(${relation.targetRow.name}) - right`;
                    return (
                        <Handle key={key} type="target" id={key} position={Position.Right} style={{ top: `${rowStartY}px`, right: '2px', ...handleStyle }} />            
                    )
                })
            }
        </div>
                    ...
                    <div style={{ borderRadius: "0 0 4px 4px", backgroundColor: "white", padding: `0 4px`, ...contentBorderStyle }}>
                        {
                            table.tableRows.map((row, i) => {
                                ...
                                return (
                                    <DrawTableRow key={row.name} tableName={table.head} tableId={table.id} row={row} rowStartY={rowStartY} height={rowHeight} />
                                );
                            })
                        }
                    </div>
